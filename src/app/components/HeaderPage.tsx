...
export default function HeaderPage(props: HeaderPageProps) {
  ...
  return (
    <Header className="flex flex-col items-center lg:flex-row lg:gap-4">
      <div className="flex w-full shrink-0 flex-row items-center gap-4 lg:w-fit">
        ...
        {!!props.icon && (
          <div className="flex h-[150px] w-[150px] items-center justify-center rounded bg-gray-100">
      ...
      <div className="flex flex-col gap-4 p-4">
        <h1 className="font-heading baseline -mb-2 inline-flex items-center gap-2 text-3xl text-slate-700">
          <span>{props.title}</span>
          {props.number && (
            <Badge variant="outline" className="text-muted inline text-base">
              {props.number}
        ...
        <div className="max-w-full">{props.subtitle}</div>
      </div>
    </Header>
  )
}
