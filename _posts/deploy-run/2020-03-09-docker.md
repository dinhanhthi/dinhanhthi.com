...
keywords: "pybash tania rascia CI CD continuous integration deployment pipeline docker idea how to use airflow kubernetes k8s k apache"
---
...
- `WORKDIR`{:.tpink}: change directory when you build the image (install this on that directory, for example).
...
Build an image ubuntu containing installed python and run jupyter notebook at the beginning. If you wanna integrate GPU in docker container, check [this section](#make-nvidia-works-in-docker).
