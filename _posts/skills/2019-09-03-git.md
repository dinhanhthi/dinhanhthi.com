...
If there are changes from both local and remote, there will be conflicts! Something like that,
...
git merge template/[branch to merge]
    ~~~
5. If there is error `fatal: refusing to merge unrelated histories`, try to add `--allow-unrelated-histories`. There must be conflict.
