...
Exclude some **folders** (we cannot use `git` in this case): Create a file called `rm_folders.sh`
...
~~~ bash
git merge <from-branch> && sh rm_folders.sh
~~~
...
# on windows, remove `''`
    ~~~
6. If there are **folders** in `dinhanhthi.com/notetheme2` you wanna keeo, add below line to `rm_folders.sh` (under branch `dinhanhthi.com/notetheme2`) before merging,
    ~~~ bash
#!/bin/sh
rm -rf /folder-1
rm -rf /folder-2
    ~~~
    and then run `git merge master && sh rm_folders.sh`.
    {% hsbox My personal .sh file %}

    {% endhsbox %} 
1. Update changes from `dinhanhthi.com/notetheme2` to repo `notetheme2`{% ref https://stackoverflow.com/questions/56577184/github-pull-changes-from-a-template-repository/56577320 %}.
    ~~~ bash
